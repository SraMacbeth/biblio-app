...
class BooksView(BaseView):
    ...
    def __init__(self, parent, controller, user=None):
        ...
        text_label_updated = self.search_bar.set_text_label(
        ...
        self.edit_button = Button(
            ...
            padx=5,
            pady=5,
            command=self.open_edit_book_form,
            state=DISABLED)
        self.edit_button.grid(row=0, column=0, padx=5, pady=5)

        self.new_book_button = Button(
            self.main_area,
            text="Agregar nuevo libro",
            command=self.open_new_book_form)
    ...
    def search(self):
        ...
        if result["estado"] == "ok":
            ...
            self.search_result_container.adjust_columns_to_content(
